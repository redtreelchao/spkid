...
<div class="toolbar">
     <div class="toolbar-inner row no-gutter hu-cart-settlement">
        <span id="h_goods_total" style="display: none;"><?php print fix_price($sub_info->product_price*$sub_info->buy_num); ?></span>
            <div class="col-60"><a href="#" class="link cart_del" style="color:#666;">实付款：<div style="color:#333; font-size:1.2em;">￥<span id="h_unpay_total"><?php print fix_price($sub_info->product_price*$sub_info->buy_num + $shipping_fee - $voucher_amount); ?></span></div></a></div>
...
<div class="page-content article-bg" style="padding-bottom:80px;">
    ...
    <div class="list-block media-list sure-order" style="margin-top:10px; margin-bottom:0;">
        <ul class="hu-qrdds-order">
          ...
          <li>
              <a href="/pdetail-<?=$sub_info->product_id;?>.html" class="external item-link item-content sure-order-jl">
                <div class="item-media col-v-img">
                    <img src="<?php print img_url($sub_info->img_url); ?>.85x85.jpg" alt="<?php print $sub_info->brand_name . ' ' . $sub_info->product_name; ?>"  />
                </div>
              <div class="item-inner">
                <div class="public-text"><?php print $sub_info->brand_name . ' ' . $sub_info->product_name; ?></div>
                <div class="hu-gwc">规格：<span data-subid="<?=$sub_info->sub_id;?>"><?php print $sub_info->size_name; ?><span></div>
                <div class="item-title-row" style="background-image:url();">
                    <div class="hu-cart-number">X<span><?php print $sub_info->buy_num; ?></span></div>
                    <div class="guanzhu-jiage">￥<?php print fix_price($sub_info->product_price*$sub_info->buy_num); ?></div>
        ...
        <div class="order-details-rr">
	 		<a href="#" class="item-link item-content open-picker" data-picker=".picker-shipping" style="padding-left:5px;">
	      		<div class="not-paid-wr clearfix">	        
                    <div class="">配送方式：
		      			<span id="default_shipping" data-id="<?=$shipping_list[$shipping_id]->shipping_id?>"><?=$shipping_list[$shipping_id]->shipping_name?></span> ￥<span id="shipping_fee"><?=$shipping_fee?></span>元
		     		</div>                
	      		</div>
	      		<div class="not-paid-jt"></div>
	      	</a>
   ...
   <div class="not-paid hu-xjqs">
         ...
         <div class="clearfix" style="padding-bottom:10px;">       
	      ...
	      <div class="not-paid-wr clearfix">
                 ...
                 使用现金券<span id="h_use_voucher" style="margin-left:10px; color:#333;">￥<?=$voucher_amount?></span> 
	    ...
	    <div class="not-paid-jt"></div>
            ...
            <div class="default-address-hu clearfix" style="padding:0 0;">  
               <div class="clearfix" style="padding-top:10px; float:left;">              
		           ...
		           <span id="h_used_money" class="guanzhu-jiage2">￥<?=$balance?></span>                  
               </a>                   
	          </div>
                    ...
                    <div class="tabs cash-coupon">
                         <div id="cat_0" class="tab active">
                             ...
                             <div class="cash-coupon1 clearfix">
                                ...
                                <?php if(isset($payment['voucher']['product'][$sub_info->product_id]) && $payment['voucher']['product'][$sub_info->product_id]->voucher_sn == $v->voucher_sn): 
                         ...
                         <div id="cat_1" class="tab">
                             <ul class="manually clearfix">
                                  <?php if(isset($payment['voucher']['product'][$sub_info->product_id]) && !$coupon_input_flag): 
                              $key = array_keys($payment['voucher']['product']);
                              ?>
                                 <li><span class="manually-bk"><input id="h_voucher_input" type="text" class="manually-input" placeholder="请输入券号" value="<?=$payment['voucher']['product'][$sub_info->product_id]->voucher_sn?>" disabled></span><span class="manually-btn"><a href="#" id="use_coupon" class="coupon-button2">取消使用</a></span></li>
                                 <?php else: ?>                                 
    ...
    <div class="picker-modal-inner">
		...
		<div class="order-details-rr clearfix">
		     <div class="hu-kd-bt item-title"><?=$shipping2->shipping_name?></div>
                     <?php if($shipping_id == $shipping2->shipping_id): ?>
...
<div class="picker-modal picker-invoice">
    ...
    <div class="picker-modal-inner">
        <div class="order-details-feiyong2">
	     ...
	     <li class="invoice_list">
		     <div class="order-details-rr clearfix">
		          <div class="hu-kd-bt"><?php //echo $invoice->title?></div>
...
var v_product_id = '<?=$sub_info->product_id?>';
var v_buy_num = '<?=$sub_info->buy_num?>';
var v_sub_id = '<?=$sub_info->sub_id?>';
var v_cur_coupon = '<?php if(isset($payment['voucher']['product'][$sub_info->product_id])){echo $payment['voucher']['product'][$sub_info->product_id]->voucher_sn;}?>';
...
function use_voucher(voucher_sn, use_type, p_obj) {
    ...
    $$.ajax({
        url: '/cart/pay_voucher_product',
        data: {product_id: v_product_id, num:v_buy_num, voucher_sn: voucher_sn, rnd: new Date().getTime()},
        dataType: 'json',
        type: 'POST',
        success: function(result) {
            if (result.msg) {
...
$$(document).on('click', '.address-list', function (e) {
    ...
    $$.ajax({
        url: '/cart/get_shipping_fee3/'+shipping_id+'/'+id+'/'+v_product_id+'/'+v_buy_num,
...
$$(document).on('click', '.shipping_list', function (e) {
    ...
    $$.ajax({
        url: '/cart/get_shipping_fee3/'+id+'/'+address_id+'/'+v_product_id+'/'+v_buy_num,
        data: {rnd: new Date().getTime()},
        dataType: 'json',
	async:true,
        type: 'POST',
...
$$(document).on('click', '.tack-invoice', function (e) {
    if ($$("span", this).hasClass('chooseone2')){
        //alert(1);
        $$("span", this).removeClass('chooseone2');
        $$(".invoice-looked").hide();
    } else {
        //alert(2);
        $$("span", this).addClass('chooseone2');
        $$(".invoice-looked").show();
    }
...
function j_voucher_use(v_voucher_sn){
    $$.ajax({
        url: '/cart/pay_voucher_product',
        data: {product_id: v_product_id, num:v_buy_num, voucher_sn: v_voucher_sn, rnd: new Date().getTime()},
        dataType: 'json',
	async:true,
        type: 'POST',
        success: function(result) {
...
function submit_cart() {
    ...
    $$.ajax({
        url:'/cart/proc_checkout_product/'+v_sub_id,
        data:data,
        dataType:'json',
        type:'POST',
        success:function(result){
