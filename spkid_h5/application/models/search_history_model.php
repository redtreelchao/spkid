...
class Search_history_model extends CI_Model {
    ...
    if( $row = $this->filter( array('keyword'=> $data['keyword']) ) ){
        return $row->id;
        }
        else 
        $this->db->insert('ty_search_history', $data);
        return $this->db->insert_id();
    }

    public function filter($filter) {
        $query = $this->db_r->get_where('ty_search_history', $filter, 1);
        return $query->row();
    }

    public function update($data, $model_id) {
        $this->db->update('ty_search_history', $data, array('id' => $model_id));
    }

    public function del($data) {
        $this->db->delete('ty_search_history', $data);
    }
