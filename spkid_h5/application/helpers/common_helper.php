...
if (!function_exists('get_nav_subtype_brand')) {
	function get_nav_subtype_brand() {
		...
		foreach ($nav_subtype_arr as $cate_id => $row) {
			...
			// 导航下热销品牌
			$bhtml = "";
			if (!empty($cate_topten_brand_arr[$cate_id])) {
				$bhtml = "<dl class='group_brand'><dt><dfn class='fb'>热卖品牌</dfn></dt>";
			}
		...
		if (!empty($nav_goodstype)) {
			$data['nav_subtype'] = json_encode($nav_goodstype);
		}

		if (!empty($nav_subtype_arr)) {
			$data['goods_type'] = $nav_subtype_arr;
		}

		if (!empty($nav_brand)) {
			$data['nav_brand'] = json_encode($nav_brand);
		}
...
function isReqFromWechat() {
	$CI = &get_instance();
	return stripos($CI->input->server('HTTP_USER_AGENT'), 'MicroMessenger') !== false;
}
