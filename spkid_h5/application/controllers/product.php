...
class Product extends CI_Controller {
	...
	public function info($param) {
		...
		if (!$p || $is_preview) {
			...
			if ($p->genre_id == PRODUCT_TOOTH_TYPE) {
				return $this->pdetail($param);
			}
...
//			$p->product_desc=adjust_path($p->product_desc);
		...
		if (!$use_cache_sub || $is_preview || ($sub_list = $this->cache->get($cache_key)) === FALSE) {
			...
			//save
			if ($use_cache_sub && !$is_preview) {
				$this->cache->save($cache_key, $sub_list, CACHE_TIME_PRODUCT_SUB);
			}
...
*/
		...
		if ($p->genre_id == PRODUCT_COURSE_TYPE) {
//科程商品
...
//			'url_map'	=> $url_map,
	...
	public function pdetail($param) {
		...
		$is_preview = isset($_GET['is_preview']) && $_GET['is_preview'] == 1 ? TRUE : FALSE;
		if (!$ghost) {
			$p = $this->cache->get('pdetail-' . $product_id);
		}
		...
		if (!$p || $is_preview) {
			...
			if (!$p || (!$is_preview && !$p->is_audit && !$ghost)) {
				redirect('index');
			}
			...
			if (!$ghost && !$is_preview) {
				$this->cache->save('pdetail-' . $product_id, $p, CACHE_TIME_PRODUCT);
			}
		...
		if (!$is_preview && !$p->is_audit && !$ghost) {
			redirect('index');
		}
...
//			'gallery'	=>$gallery,
	...
	public function brand($param) {
		...
		*/
		...
		if (!isset($age_filter[$param['age']])) {
			$param['age'] = 0;
		}

		//相关brand
		$other_brand = index_array(memcache_get_brand_list(), 'brand_id');
		if (isset($other_brand[$param['brand_id']])) {
			unset($other_brand[$param['brand_id']]);
		}
	...
	public function nav($param) {
		...
		if (!isset($nav_list[$param['nav_id']])) {
			redirect('index');
		}
		...
		foreach ($category_ids as $category_id) {
			if (!isset($category_number[$category_id])) {
				continue;
			}
			...
			foreach ($category_number as $c) {
				if ($c->parent_id == $category_id && $c->number) {
					$category->sub_category[] = $c;
				}
		...
		foreach ($category_list as $cat) {
			foreach ($cat->sub_category as $c) {
				if (isset($category_brand[$c->category_id])) {
					$brand_filter += $category_brand[$c->category_id];
				}
	...
	*/
	...
	public function get_brand_story($brand_id) {
		...
		if (!$brand) {
			$brand = array('brand_name' => '', 'brand_logo' => '', 'brand_story' => '');
		}
	...
	public function search() {
		$data = array();
		// get all hotwords
                $type = intval($this->input->get('stype'));
		$this->load->model('hotword_model');
		$data['hotwordlist'] = $this->hotword_model->all(array('hotword_type' => $type));
                $data['type'] = $type;
	...
	public function searchResult() {
		...
		$ids = $this->input->get('ids');
                $search_type = intval($this->input->get('search_type'));
		$sort = $this->input->get('sort', true);
		$page = intval($this->input->get('page'));
		//对于一级关键字种类进行搜索的时候，将关键字搜素转化为一级分类id进行搜索
		$this->load->model('product_type_model');
		$cache_first_class = 'first_classes';
		$is_preview = intval($this->input->get('is_preview'));

		if (isset($is_preview) && $is_preview === 1) {
			$first_classes = $this->product_type_model->get_first_classes();
			$this->memcache->save($cache_first_class, $first_classes, CACHE_TIME_CATLIST);

		} else {
			if (($first_classes = $this->memcache->get($cache_first_class)) === FALSE) {
				$first_classes = $this->product_type_model->get_first_classes();
				$this->memcache->save($cache_first_class, $first_classes, CACHE_TIME_CATLIST);
			}
		}
		//用于区分是否按照分类查询还是按照关键字查询
		$searchByType = false;
		$type_id = -1;
		//内部函数，用于判断查找的关键字是否在一级分类的名称中
		function findType_id($arr, $val) {
			$type_id = -1;
			foreach ($arr as $k => $v) {
				if (in_array($val, $v)) {
					$type_id = current($v);
		...
		if (($type_id = findType_id($first_classes, $kw)) != -1) {
			$searchByType = true;
		}

		$this->load->model('rush_model');
		if ($searchByType) {
			$param = array('type_id' => $type_id, 'sort' => $sort ? $sort : 0, 'page' => $page ? $page : 0);
		} else {
			if ($ids) {
		...
		$ad = $this->lib_ad->get_ad_by_position_tag('search_result_top', 'search_result_top', 1);
                ...
                if ($search_type == 1){
                    $view_page = 'mobile/product/courseResult';
                } else {
                    $view_page = 'mobile/product/searchResult';
                }
	...
	function ajax_product_list($page_name) {
		...
		$type_id = $this->input->get('type_id');
                $stype = intval($this->input->get('stype'));
		$sort = $this->input->get('sort', true);
		//对type_id和关键字分开搜索
		...
		if ($page_name == 'searchResult') {
			...
			// }
                        ...
                        if ($stype == 1) {
                            $result['data'] = $this->load->view('mobile/index/course',array('courses'=>$data['list']), true);
                        } else {
	...
	public function searchResultByType() {
		$type_id = $this->input->get('type_id', true);
                $stype = intval($this->input->get('stype', true));
		$is_preview = intval($this->input->get('is_preview', true));
		$kw = trim($this->input->get('kw', true));
		$this->load->library('memcache');
		$sort = intval($this->input->get('sort', true));
		$page = intval($this->input->get('page'));
		$this->load->model('rush_model');
		$param = array('type_id' => $type_id, 'sort' => $sort, 'page' => $page);
		$cache_key = 'searchResultByType-' . implode('-', $param);
	...
	public function tuangou($tuan_id) {

		$tuan_id = intval($tuan_id);
		$this->load->model('tuangou_model');
		$tuan_info = $this->tuangou_model->filter(array('tuan_id' => $tuan_id));
		if(empty($tuan_info)) {
		    sys_msg('记录不存在', 1);
		}
		...
		//获取商品信息
		$p = $this->product_model->filter(array('product_id'=>$tuan_info->product_id));
		...
		// 取颜色
		$g_list = array();
		foreach ($this->product_model->all_gallery(array('product_id' => $tuan_info->product_id)) as $g) {
			if (!isset($g_list[$g->color_id])) {
				$g_list[$g->color_id] = array('default' => NULL, 'tonal' => NULL, 'part' => array());
			}
		...
		$this->load->vars('product', $p);

		//var_export($tuan_info);exit;
		$this->load->view('mobile/product/tuangou',
			array(
				'title' => $tuan_info->tuan_name,
				'p' => $p,
				'g_list' => $p->g_list
			)
		);		
