...
(function () {
    ...
    window.Framework7 = function (params) {
        ...
        ************   Modals   ************
        ...
        app.alert2 = function (text, title, btnName, callbackOk) {
            if (typeof title === 'function') {
                callbackOk = arguments[1];
                title = undefined;
            }
			btnName = typeof btnName === 'undefined' ? app.params.modalButtonOk : btnName;
            return app.modal({
                text: text || '',
                title: app.params.prefixTitle+(typeof title === 'undefined' ? app.params.modalTitle : title),
                buttons: [ {text: btnName, bold: true, onClick: callbackOk} ]
            });
        };
        ...
        app.confirm2 = function (text, title, btnNameOk,btnNameCancel, callbackOk, callbackCancel) {
            if (typeof title === 'function') {
                callbackCancel = arguments[2];
                callbackOk = arguments[1];
                title = undefined;
            }
			btnNameOk = typeof btnNameOk === 'undefined' ? app.params.modalButtonOk : btnNameOk;
			btnNameCancel = typeof btnNameCancel === 'undefined' ? app.params.modalButtonCancel : btnNameCancel;
            return app.modal({
                text: text || '',
                title: app.params.prefixTitle+(typeof title === 'undefined' ? app.params.modalTitle : title),
                buttons: [
                    {text: btnNameCancel, onClick: callbackCancel},
                    {text: btnNameOk, bold: true, onClick: callbackOk}
                ]
            });
        };
		...
		app.yywLogin = function (text, title, callbackOk, callbackCancel, css) {
            if (typeof title === 'function') {
                callbackCancel = arguments[2];
                callbackOk = arguments[1];
                title = undefined;
            ...
            return app.modal({
                ...
                '<input type="text" name="username" placeholder="手机/用户名" class="text-input" style="padding-left:10px;"></div>'+
                '<div class="input-field modal-input-double"><input type="password" name="password" placeholder="密码" class="text-input" style="padding-left:10px;"></div>'+
                '<div class="buttons-row justify-content-a"><a href="/user/signin/register-step1" class="button external">注册</a>'+
                '<a href="/user/signin/find_password" class="button external">忘记密码了?</a></div>',
                buttons: [
                    {
                        text: app.params.modalButtonCancel
                ...
                onClick: function (modal, index) {
                    var username = $(modal).find('.text-input[name="username"]').val();
                    var password = $(modal).find('.text-input[name="password"]').val();
                    if (index === 0 && callbackCancel) callbackCancel(username, password);
                    if (index === 1 && callbackOk) callbackOk(username, password);
                },
                cssClass: css//'yyw-login'
            });
        };
        ...
        app.modalPassword = function (text, title, callbackOk, callbackCancel) {
            ...
            return app.modal({
                ...
                //share modal 
                app.modalShare = function (text, title, callbackOk, callbackCancel) {
                    if (typeof title === 'function') {
                        callbackCancel = arguments[2];
                        callbackOk = arguments[1];
                        title = undefined;
                    ...
                    return app.modal({
                        text: text || '',
                        title: typeof title === 'undefined' ? app.params.modalTitle : title,
                        afterText: '<div class="jiathis_style_32x32"><a class="jiathis_button_weixin"></a><a class="jiathis_button_qzone"></a><a class="jiathis_button_cqq"></a><a class="jiathis_button_tsina"></a><a class="jiathis_counter_style"></a></div>',
                        buttons: [
                            {
                                text: app.params.modalButtonCancel
                            },
        ...
        ************   Notifications   ************
        ...
        app.addNotification = function (params) {
            ...
            if (params.additionalClass) {
                container.addClass(params.additionalClass);
            }
            if (params.hold) {
                setTimeout(function () {
    ...
    var Dom7 = (function () {
            ...
            append: function (newChild) {
                ...
                for (i = 0; i < this.length; i++) {
                    if (typeof newChild === 'string') {
						...
						else jQuery(this[i]).append(newChild);
                        //this[i].appendChild(newChild);
                        //console.log(this[i])
                    }
                }
        ...
        (function () {
            ...
            function createMethod(method) {
                $[method] = function (url, data, success,callback) {
                    return $.ajax({
                        url: url,
