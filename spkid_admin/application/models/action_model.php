...
class Action_model extends CI_Model {
    ...
    public function list_f($filter) {
        $from = " FROM ty_admin_action  ";
        $where = " WHERE 1 ";

        $param = generate_where_by_filter( $filter, USE_SQL_OR );
        if( !empty($param) ) $where .= "AND ".array_pop( $param );

        $filter['sort_by'] = empty($filter['sort_by']) ? 'action_id' : trim($filter['sort_by']);
    ...
    public function insert($data) {
        $this->db->insert('ty_admin_action', $data);
        return $this->db->insert_id();
    }

    public function filter($filter) {
        $query = $this->db_r->get_where('ty_admin_action', $filter, 1);
        return $query->row();
    }

    public function update($data, $model_id) {
        $this->db->update('ty_admin_action', $data, array('action_id' => $model_id));
    }

    public function del($data) {
        $this->db->delete('ty_admin_action', $data);
    }
