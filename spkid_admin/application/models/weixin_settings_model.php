...
class Weixin_settings_model extends CI_Model {

    public function list_f($filter) {
        $from = " FROM ty_weixin_settings";
        $where = " WHERE 1 ";
    ...
    public function insert($data) {
        $this->db->insert('ty_weixin_settings', $data);
        return $this->db->insert_id();
    }

    public function filter($filter) {
        $query = $this->db_r->get_where('ty_weixin_settings', $filter, 1);
        return $query->row();
    }

    public function update($data, $model_id) {
        $this->db->update('ty_weixin_settings', $data, array('id' => $model_id));
    }

    public function del($data) {
        $this->db->delete('ty_weixin_settings', $data);
    }
    
    public function filter_all() 
    {
        $query = $this->db->get_where('ty_weixin_settings');
