...
class Proc_log_model extends CI_Model {

    public function list_f($filter) {
        $from = " FROM ty_procedure_log";
        $where = " WHERE 1 ";
    ...
    public function insert($data) {
        $this->db->insert('ty_procedure_log', $data);
        return $this->db->insert_id();
    }

    public function filter($filter) {
        $query = $this->db_r->get_where('ty_procedure_log', $filter, 1);
        return $query->row();
    }

    public function update($data, $model_id) {
        $this->db->update('ty_procedure_log', $data, array('id' => $model_id));
    }

    public function del($data) {
        $this->db->delete('ty_procedure_log', $data);
    }
