...
class File_upload_model extends CI_Model {
    ...
    public function insert($data) {
        $this->db->insert('ty_upload_files', $data);
        return $this->db->insert_id();
    }

    public function filter($filter) {
        $query = $this->db_r->get_where('ty_upload_files', $filter, 1);
        return $query->row();
    }

    public function update($data, $model_id) {
        $this->db->update('ty_upload_files', $data, array('id' => $model_id));
    }

    public function del($data) {
        $this->db->delete('ty_upload_files', $data);
    }
