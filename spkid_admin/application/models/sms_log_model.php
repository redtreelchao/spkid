...
class Sms_log_model extends CI_Model {
    ...
    public function insert($data) {
        $this->db->insert('ty_sms_log', $data);
        return $this->db->insert_id();
    }

    public function filter($filter) {
        $query = $this->db_r->get_where('ty_sms_log', $filter, 1);
        return $query->row();
    }

    public function update($data, $model_id) {
        $this->db->update('ty_sms_log', $data, array('rec_id' => $model_id));
    }

    public function del($data) {
        $this->db->delete('ty_sms_log', $data);
    }
