...
* Order_model
*/
class Order_course_model extends CI_Model
{
	...
	public function order_list($filter)
	{
        $from = " FROM ".$this->db->dbprefix('order_info')." AS o 
				...
				";
		$where = " WHERE 1 AND o.genre_id = 2 ";
		$param = array();
	...
	if($filter['consign']){
        ...
        if($filter['product_sn']||$filter['category_id']){
	...
	public function search_product($filter)
	{
        ...
        $sql = "SELECT p.product_id,p.product_name,p.product_sn,p.promote_price,p.promote_start_date,p.promote_end_date,p.is_promote,p.shop_price,p.market_price "
