...
class Product_access_records_model extends CI_Model {

    public function list_f($filter) {
        $from = " FROM ty_product_access";
        $where = " WHERE 1 ";
    ...
    public function insert($data) {
        $this->db->insert('ty_product_access', $data);
        return $this->db->insert_id();
    }

    public function filter($filter) {
        $query = $this->db_r->get_where('ty_product_access', $filter, 1);
        return $query->row();
    }

    public function update($data, $model_id) {
        $this->db->update('ty_product_access', $data, array('id' => $model_id));
    }

    public function del($data) {
        $this->db->delete('ty_product_access', $data);
    }
