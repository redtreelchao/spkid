...
class Dict_model extends CI_Model {
    ...
    public function insert($data) {
        $this->db->insert('ty_dict_info', $data);
        return $this->db->insert_id();
    }

    public function filter($filter) {
        $query = $this->db_r->get_where('ty_dict_info', $filter, 1);
        return $query->row();
    }

    public function update($data, $model_id) {
        $this->db->update('ty_dict_info', $data, array('id' => $model_id));
    }

    public function del($data) {
        $this->db->delete('ty_dict_info', $data);
    }
