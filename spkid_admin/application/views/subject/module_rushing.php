...
<div class="main">
    <div class="main_title"><span class="l">活动专题管理 >> 模块编辑 >> 正在限抢活动 </span><a href="subject/index" class="return r">返回列表</a></div>
    ...
    <div class="search_row">
        <?php print form_open_multipart('subject/proc_edit_module',array('name'=>'mainForm'));?>
            ...
            <table id="dataTable" class="dataTable" cellpadding=0 cellspacing=0>
                <?php foreach($all_rushings as $key=>$rush): ?>
                    <?php if($key % 4 == 0): ?>
                    <tr class="row">
                    <?php endif; ?>
                        <td style="text-align: left;">
                            <input type="checkbox" name="rush_id[]" value="<?=$rush->rush_id?>" 
                                   <?php if(in_array($rush->rush_id, $selected_rushings)): ?> checked="checked" <?php endif; ?> />
                            <?=$rush->rush_index?>
                        </td>
                    <?php if($key % 4 == 3): ?>
                    </tr>
