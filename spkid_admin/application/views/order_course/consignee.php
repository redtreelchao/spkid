...
<div class="main">
	<div class="main_title"><span class="l">课程订单管理 >> <?php print $act=='add'?'新增课程订单':'编辑课程订单' ?> >> 收货地址 </span><span class="r"><a href="order_course/index" class="return">返回列表</a></span></div>
	<div class="blank5"></div>
	<div id="product_list">
		<?php include 'order_product.php' ?>
	</div>
	<?php print form_open('order_course/proc_consignee',array('name'=>'mainForm','onsubmit'=>'return check_form()'),array('order_id'=>$order->order_id,'act'=>$act));?>
		<table class="form" cellpadding=0 cellspacing=0>
			...
			<tr>
				...
				<td class="item_input">
				<?php if ($act=='add'): ?>
					<input type="button" name="myprev" value="上一步" class="am-btn am-btn-primary" onclick="location.href=base_url+'order_course/course/<?php print $order->order_id;?>?act=add'">
					
				<?php endif ?>
					<?php print form_submit('mysubmit',$act=='add'?'下一步':'确定','class="am-btn am-btn-primary"') ?>	
					<input type="button" name="mycancel" value="取消" class="am-btn am-btn-primary" onclick="location.href=base_url+'order_course/info/<?php print $order->order_id;?>'">
				</td>
			</tr>
			<tr>
