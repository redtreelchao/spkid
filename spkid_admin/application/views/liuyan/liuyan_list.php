...
<?php endif; ?>
			<table id="dataTable" class="dataTable" cellpadding=0 cellspacing=0>
			    ...
			    <tr class="row">
					...
					<td><?php print $row->tag_type==1?'商品':'礼包'; ?></td>
					<td><?php print $row->comment_type==1?'咨询':'评价'; ?></td>
					<td style="text-align:left">
					<a href="<?php print $row->tag_type==1?front_url('product-'.$row->tag_id.'.html'):front_url('package-'.$row->tag_id.'.html') ?>" target="_blank">
					<?php print $row->tag_type==1?"[{$row->product_sn}] {$row->product_name}":$row->package_name; ?>
					</a>
					</td>
