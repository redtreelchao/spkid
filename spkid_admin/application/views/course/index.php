...
<?php include(APPPATH.'views/common/header.php'); ?>
	...
	<script type="text/javascript">
		...
		function export_purcahse_course(){    
		    $.download("course/export_purcahse_course",
	...
	<div class="main">
        <div class="main_title">
        	<span class="l">课程管理 >> 课程列表</span> 
        	<span class="r"><a class="add" href="course/add">新增</a></span>
		</div>
...
<?php endif; ?>
			<table id="dataTable" class="dataTable" width="100%" cellpadding=0 cellspacing=0>
				...
				<tr class="row">
					...
					<th width="100">商品款号<br/>商品名称<br/>最后编辑时间</th>
					<th width="80">分类</th>
					<th width="80">						
				...
				<tr class="row">
					...
					<td>						
						<?php print toggle_link('course/toggle','is_best',$row->product_id, $row->is_best,'<font color=red>展品</font>','<font color=gray>展品</font>');?>
						<?php print toggle_link('course/toggle','is_new',$row->product_id, $row->is_new,'<font color=red>新品</font>','<font color=gray>新品</font>');?><br/>
						<?php print toggle_link('course/toggle','is_hot',$row->product_id, $row->is_hot,'<font color=red>热销</font>','<font color=gray>热销</font>');?>
						<?php print toggle_link('course/toggle','is_promote',$row->product_id, $row->is_promote,'<font color=red>促销</font>','<font color=gray>促销</font>');?><br/>
						<?php print toggle_link('course/toggle','is_gifts',$row->product_id, $row->is_gifts,'<font color=red>赠品</font>','<font color=gray>赠品</font>');?>						
					</td>					
					<td>
					...
					<td>
						<?php print edit_link('course/edit_field', 'sort_order', $row->product_id, $row->sort_order);?>
					</td>
					<td>
						<a class="edit" href="course/edit/<?php print $row->product_id; ?>" title="编辑"></a>
						<?php if ($perm_delete): ?><br/>
							<a class="del" href="javascript:void(0)" rel="course/delete/<?php print $row->product_id; ?>" title="删除" onclick="do_delete(this)"></a><br/>
							审核<?php print toggle_link('course/toggle','is_audit',$row->product_id, $row->is_audit);?><br>
						<?php endif ?>
									...
									<table  class="dataTable" width="100%" cellpadding=0 cellspacing=0>
											...
											<?php foreach ($row->cs_list as $sub_color): ?>
												<?php foreach ($sub_color['sub_list'] as $sub): ?>
													<td>
														...
														[<?php print edit_link('course/edit_field_sub', 'consign_num', $sub->sub_id, $sub->consign_num==-2?'不限':($sub->consign_num==-1?'不':$sub->consign_num));?>]
													</td>
												<?php endforeach ?>
											<?php endforeach ?>
										</tr>
										<tr>
											<?php foreach ($row->cs_list as $sub_color): ?>
												<?php foreach ($sub_color['sub_list'] as $sub): ?>
													<td>
														<?php print toggle_link('course/toggle_sub','is_on_sale',$sub->sub_id, $sub->is_on_sale,'<font color=red>上架</font>','<font color=gray>下架</font>');?>
													</td>
												<?php endforeach ?>
											<?php endforeach ?>
										</tr>
