...
<div class="main">
  <div class="blank5"></div>
	<?php print form_open_multipart('box_onsale/onsale',array('name'=>'mainForm','id'=>'mainForm'));?>
		<table class="form" cellpadding=0 cellspacing=0>
			...
			<tr>
				...
				<tr>
				<td class="item_title">单号:</td>
				<td class="item_input">
					<span id="do_code"></span>
				</td>
			</tr>
			<tr>
		...
		<table class="form" cellpadding=0 cellspacing=0>
			<tr>
				<td class="item_input" style="padding: 20px;">
					...
					<?php else :?>
					<span style="color: red" id="data_msg"><?php if(!empty($msg))echo $msg;?></span>
					<?php endif;?>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		var doc_code = '<?php echo $doc_code;?>';
		var box_code = '';
		<?php if(!empty($box_code)):?>
		box_code = '<?php echo $box_code;?>';
		<?php endif;?>
		if(doc_code !=""){
			$("#doc_code").val(doc_code);
			$("#do_code").html(doc_code);
			if(box_code !=""){
		...
		$("#data_code").keydown(function(event){
			...
			if(event.which == 13){
				...
				if(data_code == "" || data_code == null){
				    ...
				    if(doc_code == "" || doc_code == null){
						$.post("/box_onsale/check_doc_code",
						{is_ajax:1,doc_code:data_code,doc_type:doc_type,rnd : new Date().getTime()},
						function(data){
							...
							if(data.err == 1){
							    ...
							    $("#doc_code").val(data_code);
							    $("#do_code").html(data_code);
							}
						});
						$("#data_code").val('');
						$("#data_code").focus();
					}else if(box_code == "" || box_code == null){
						$.post("/box_onsale/check_box_code",
						{is_ajax:1,doc_code:doc_code,doc_type:doc_type,box_code:data_code,rnd : new Date().getTime()},
						function(data){
							data = jQuery.parseJSON(data);
							if(data.err == 1){
							    alert(data.msg);
