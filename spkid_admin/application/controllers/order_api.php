...
class Order_api extends CI_Controller
{
	...
	public function search_user()
	{
		...
		$user_list = $this->order_model->search_user($user_name);
	...
	public function search_product()
	{
		...
		$data['content'] = $this->load->view('order/search_product', $data, TRUE);
		$data['error'] = 0;
		unset($data['list']);
	...
	public function add_product()
	{
		...
		$product = $this->order_model->filter_product(array('order_id'=>$order_id,'product_id'=>$product_id,'color_id'=>$color_id,'size_id'=>$size_id,'package_id'=>0,'discount_type !='=>4));
...
*/
		...
		$update = array(
			...
			'product_price'=>$price,
			//'provider_cess'=>$product->goods_cess,
		...
		$result = array('err'=>0,'msg'=>'','data'=>$this->load->view('order/order_product',array('edit_product'=>TRUE),TRUE));
	...
	public function remove_product()
	{
		...
		$payment = $this->order_model->filter_payment(array('order_id'=>$order_id,'pay_id'=>PAY_ID_VOUCHER));
		if($payment) sys_msg('请先取消折扣支付，如现金券',1);
		$product = $this->order_model->filter_product(array('order_id'=>$order_id,'op_id'=>$op_id,'package_id'=>0));
		if(!$product) sys_msg('记录不存在',1);
		if($product->discount_type==4) sys_msg('赠品不能手动删除');
		//取库存记录
		$sub = $this->product_model->lock_sub(array('product_id'=>$product->product_id,'color_id'=>$product->color_id,'size_id'=>$product->size_id));
		if(!$sub) sys_msg('没有库存记录',1);
		
		//删除商品
		$this->order_model->delete_product($op_id);		
	...
	public function switch_lock()
	{
		...
		//$this->order_model->insert_action($order,$op=='lock'?'锁定':'解锁');
		$this->db->trans_commit();
		...
		$update['odd'] = 1;
		$this->order_model->update($update,$order_id);
		$this->order_model->insert_action($order,'将订单标记为问题单');
		$this->db->trans_commit();
		print json_encode(array('err'=>0,'msg'=>''));
        }
		...
		$update['odd'] = 0;
		$this->order_model->update($update,$order_id);
		$this->order_model->insert_action($order,'取消该订单的问题单标记');
		$this->db->trans_commit();
		print json_encode(array('err'=>0,'msg'=>''));
        }
	...
	public function post_advice()
	{
		...
		$order = $this->order_model->filter(array('order_id'=>$update['order_id']));
		if(!$order) sys_msg('记录不存在', 1);
		$perms=get_order_perm($order);
		...
		$this->order_model->insert_advice($update);
		print json_encode(array('err'=>0,'msg'=>''));
	}
	...
	public function pay_voucher()
	{
		...
		$this->order_model->update(array('paid_price'=>fix_price($order->paid_price)),$order_id);
		update_shipping_fee($order);
		$this->db->trans_commit();
		print json_encode(array('err'=>0,'msg'=>''));
	}
	...
	public function invalid()
	{
		...
		if(!$order_payment && !$order_product){
			...
			$this->order_model->delete_advice_where(array('order_id'=>$order_id,'is_return'=>0));
			$this->order_model->delete_action_where(array('order_id'=>$order_id,'is_return'=>0));
		}else{
			$delete = FALSE;
			...
			$this->order_model->insert_action($order,'订单作废');
		}
		$this->db->trans_commit();
	...
	public function free_shipping_fee()
	{
		...
		$perms = get_order_perm($order);
		if(!$perms['edit_order']) sys_msg('不能操作',1);
		$this->order_model->update(array('shipping_fee'=>0),$order_id);
		$this->order_model->insert_action($order,'免运费');
		$this->db->trans_commit();
		print json_encode(array('err'=>0,'msg'=>''));
	}
	...
	public function reset_shipping_fee()
	{
		...
		if(!$perms['edit_order']) sys_msg('不能操作',1);
		update_shipping_fee($order);
		$this->order_model->insert_action($order,'重新计算运费');
		$this->db->trans_commit();
		print json_encode(array('err'=>0,'msg'=>''));
	}
		...
		if($order->order_amount<0){
			...
			$update['paid_price'] = fix_price($order->paid_price + $order->order_amount);
                ...
                if (in_array($order->source_id, array_keys($third_parts))) {
                        ...
                        $this->order_model->update(array('paid_price'=>$paid_price), $order->order_id);
	...
	public function unconfirm()
	{
		...
		$this->order_model->update($update,$order_id);
		foreach($update as $key=>$val) $order->$key = $val;
		$this->order_model->insert_action($order,'订单反审核');
		$this->db->trans_commit();
		print json_encode(array('err'=>0,'msg'=>''));
	}
	...
	public function shipping()
	{
		...
		$product = $this->order_model->filter_product(array('order_id'=>$order->order_id,'consign_num !='=>0));
		if($product){
			if($shipping_true) {
	...
	public function payment()
	{
		...
		$available_pay = $this->order_model->available_pay(array('source_id'=>$order->source_id,'shipping_id'=>$order->shipping_id));
		...
		// 更新订单表
		$this->order_model->update(array('paid_price'=>fix_price($order->paid_price+$payment_money)),$order_id);
		$this->db->trans_commit();
		print json_encode(array('err'=>0,'msg'=>''));
	...
	public function delete_payment()
	{
		...
		$perms = get_order_perm($order);
		if(!$perms['edit_pay']) sys_msg('不能操作',1);
		$payment = $this->order_model->filter_payment(array('order_id'=>$order_id,'payment_id'=>$payment_id,'is_return'=>0));
		if(!$payment) sys_msg('支付记录不存在',1);
		// 删除支付记录
		$this->order_model->delete_payment($payment_id);
		// 更新订单表
		$this->order_model->update(array('paid_price'=>fix_price($order->paid_price-$payment->payment_money)),$order_id);
		$this->db->trans_commit();
		print json_encode(array('err'=>0,'msg'=>''));
	}
	...
	public function edit_price()
	{
		...
		$order_product = index_array($this->order_model->order_product($order_id),'op_id');
		...
		if($this->order_model->filter_payment(array('order_id'=>$order_id,'pay_id'=>PAY_ID_VOUCHER))) sys_msg('请先取消现金券',1);

		// 修改订单商品表
		$this->order_model->update_product(array(
			...
			'discount_type' => 3
                ...
                //修改订单出入库信息表
                $this->order_model->update_transaction (
                        array('shop_price'=>$new_price),
                        array('trans_sn'=>$order->order_sn,'product_id'=>$op->product_id,'color_id'=>$op->color_id,'size_id'=>$op->size_id)
                        );
		...
		$this->order_model->insert_action($order,"修改商品 {$op->product_name} {$op->color_name} {$op->size_name} 价格，由 {$op->product_price} 改为 {$new_price} 【{$reason}】");
		$this->db->trans_commit();
		check_gifts($order_id);
		print json_encode(array('err'=>0,'msg'=>''));
	}
	...
	public function load_package()
	{
		...
		$order_product = array();
		if($extension_id){
			foreach ($this->order_model->order_product($order_id) as $p) {
	...
	public function add_package()
	{
		...
		foreach ($product_list as $p) {
			$op_id = $this->order_model->insert_product(array(
				'order_id'=>$order_id,
			...
			if(!$extension_id){
				$extension_id = $op_id;
				$this->order_model->update_product(array('extension_id'=>$extension_id),$op_id);
			}
			$info = $this->order_model->assign_trans($order,$p,$p->gl_num,$op_id);
	...
	public function remove_package()
	{
		...
		foreach ($package_product as $p) {
			...
			$this->order_model->delete_product($p->op_id);
			// 更新订单主表
			$order->product_num -= 1;
	...
	public function edit_invoice_no()
	{
		...
		$this->order_model->update(array('invoice_no'=>$invoice_no),$order_id);
		$this->order_model->insert_action($order,'更改运单号为'.$invoice_no);
		$this->db->trans_commit();
		print json_encode(array('err'=>0,'msg'=>''));
	}
	...
	public function edit_real_shipping_fee()
	{
		...
		if(!$order) sys_msg('订单不存在',1);
		if(!$order->shipping_status) sys_msg('不能操作',1);
		$this->order_model->update(array('real_shipping_fee'=>$real_shipping_fee),$order_id);
		$this->order_model->insert_action($order,'更改实际运费为'.number_format($real_shipping_fee,2));
		$this->db->trans_commit();
		print json_encode(array('err'=>0,'msg'=>''));
	}
