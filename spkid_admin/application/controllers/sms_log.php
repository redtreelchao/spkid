...
class Sms_log extends CI_Controller {
    ...
    public function index() {
        ...
        $data['full_page'] = TRUE;
        $this->load->view('sms_log/index', $data);
    }
    ...
    public function delete($pk_id) {
        auth('sms_log_delete');
        $pk_id = intval($pk_id);
        $check = $this->sms_log_model->filter(array('rec_id' => $pk_id));

        if (empty($check)) {
            sys_msg('记录不存在', 1);
            return;
        }
        $this->sms_log_model->del(array('rec_id' => $pk_id));
        sys_msg('操作成功', 2, array(array('href' => 'sms_log/index', 'text' => '返回列表页')));
    }
    ...
    public function editable() {
        ...
        $data[$name] = $value;
        $result = $this->sms_log_model->update( $data, $pk );
