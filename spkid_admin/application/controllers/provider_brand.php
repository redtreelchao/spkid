...
class Provider_brand extends CI_Controller
{
	...
	public function index ($provider_id)
	{
		...
		$data = $this->provider_brand_model->brand_list($filter);
		if ($this->input->is_ajax_request())
		{
		    $data['full_page'] = FALSE;
		    $data['content'] = $this->load->view('provider/provider_brand', $data, TRUE);
		    $data['error'] = 0;
		    unset($data['list']);
		...
		$provider_brand_list = $this->provider_brand_model->provider_brand_list($provider_id);
		$data['brand_list'] = $provider_brand_list;
		$data['full_page'] = TRUE;
		$this->load->vars('row', $provider);
		$this->load->view('provider/provider_brand',$data);
	}
	...
	public function add_brand(){
		...
		echo json_encode(array("err"=>1,"msg"=>"没有供应商或者品牌。"));
		return;
	    }
	    $provider = $this->provider_model->filter(array('provider_id'=>$provider_id));
	    if (empty($provider)) {
		echo json_encode(array("err"=>1,"msg"=>"系统中不存在对应供应商。"));
		return;
	    }
	    ...
	    $provider_brand = $this->provider_brand_model->filter(array("provider_id"=>$provider_id,"brand_id"=>$brand_id));
	    if(!empty($provider_brand)){
		echo json_encode(array("err"=>1,"msg"=>"供应商已经关联此品牌",));
		return;
	    }
	    $update = array();
	    ...
	    $this->provider_brand_model->insert($update);
	...
	public function remove_brand(){
	    ...
	    $provider_brand = $this->provider_brand_model->filter($filter);
	    if(!empty($provider_brand)){
		$this->provider_brand_model->delete($provider_brand->id);
	    }
	    ...
	    $update['commission'] = trim($this->input->post('commission'));
            ...
            $this->provider_brand_model->update($update, $id);
