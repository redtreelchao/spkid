function search_user () {
	...
	$.ajax({
		url:'order_course_api/search_user',
...
function submit_add_product () {
	...
	if (act=='add') {
		location.href=base_url+"order_course/consignee/"+order_id+'?act=add';
	}else{
		location.href=base_url+"order_course/info/"+order_id;
	}
}
...
function switch_lock (op) {
	var order_id = $(':hidden[name=order_id]').val();
	$.ajax({
		url:'order_course_api/switch_lock',
...
function odd_cancel()
{
    ...
    $.ajax({
            url:'order_course_api/odd_cancel',
            data:{order_id:order_id,rnd:new Date().getTime()},
            dataType:'json',
            type:'POST',
            success:function (result) {
...
function post_advice () {
	...
	$.ajax({
		url:'order_course_api/post_advice',
...
function add_product (product_id) {
    ...
    $.ajax({
    	url:'order_course_api/add_product',
    	data:{order_id:order_id,product_id:product_id,num:num,rnd:new Date().getTime()},
    	dataType:'json',
    	type:'POST',
    	success:function(result){
    		if(result.msg) alert(result.msg);
...
function remove_product (op_id) {
	var order_id = $(':hidden[name=order_id]').val();
	$.ajax({
		url:'order_course_api/remove_product',
...
function pay_balance (argument) {
	...
	$.ajax({
		url:'order_course_api/pay_balance',
...
function pay_voucher () {
	...
	$.ajax({
		url:'order_course_api/pay_voucher',
...
function remove_voucher () {
	var order_id = $(':hidden[name=order_id]').val();
	$.ajax({
		url:'order_course_api/remove_voucher',
		data:{order_id:order_id,rnd:new Date().getTime()},
		dataType:'json',
		type:'POST',
		success:function(result){
...
function invalid () {
	...
	$.ajax({
		...
		success:function(result){
			...
			if(result.err) return false;
			if(result.redirect)
				location.href = $('base').attr('href')+'order_course/';
			else
				location.href=location.href;
		}
	});
...
function free_shipping_fee () {
	var order_id = $(':hidden[name=order_id]').val();
	$.ajax({
		url:'order_course_api/free_shipping_fee',
		data:{order_id:order_id,rnd:new Date().getTime()},
		dataType:'json',
		type:'POST',
		success:function(result){
...
function reset_shipping_fee () {
	var order_id = $(':hidden[name=order_id]').val();
	$.ajax({
		url:'order_course_api/reset_shipping_fee',
		data:{order_id:order_id,rnd:new Date().getTime()},
		dataType:'json',
		type:'POST',
		success:function(result){
...
function reset_saler () {
    ...
    $.ajax({
            url:'order_course_api/reset_saler',
...
function update_shipping_fee () {
	...
	$.ajax({
		url:'order_course_api/update_shipping_fee',
...
function order_unconfirm () {
	var order_id = $(':hidden[name=order_id]').val();
	$.ajax({
		url:'order_course_api/unconfirm',
		data:{order_id:order_id,rnd:new Date().getTime()},
		dataType:'json',
		type:'POST',
		success:function(result){
...
function order_shipping (shipping_true,invoice_no) {
   // if ($('#h_shipping').dialog('isOpen')) {
        ...
        var logistics = $('[type=hidden][name=v_logistics]').val();
        var mobile = $('[type=hidden][name=v_mobile]').val();
        ...
        $.ajax({
            url:'order_course_api/shipping',
            data:{order_id:order_id,shipping_true:shipping_true,invoice_no:invoice_no,logistics:logistics,mobile:mobile,rnd:new Date().getTime()},
            dataType:'json',
            type:'POST',
            success:function(result){
                if(result.confirm){
...
function remove_payment (payment_id) {
	var order_id = $(':hidden[name=order_id]').val();
	$.ajax({
		url:'order_course_api/delete_payment',
...
function order_deny () {
	var order_id = $(':hidden[name=order_id]').val();
	location.href=$('base').attr('href')+'order_course/deny/'+order_id;
}
...
function edit_price (op_id) {
	...
	$.ajax({
		url:'order_course_api/edit_price',
...
function load_package (package_id,extension_id) {
	var order_id = $(':hidden[name=order_id]').val();
	$.ajax({
		url:'order_course_api/load_package',
...
function add_package () {
	...
	$.ajax({
		url:'order_course_api/add_package',
...
function remove_package (extension_id) {
	var order_id = $(':hidden[name=order_id]').val();
	$.ajax({
		url:'order_course_api/remove_package',
...
function change_source () {
	...
	if(!source_id) return;
	$.ajax({
		url:'order_course/load_pay_list',
...
function change_pay () {
	...
	if(!pay_id) return;
	$.ajax({
		url:'order_course/load_shipping_list',
...
function change_routing () {
	...
	$.ajax({
		url:'order_course/change_routing',
...
function edit_invoice_no(){
	...
	$.ajax({
		url:'order_course_api/edit_invoice_no',
...
function edit_real_shipping_fee(){
	...
	$.ajax({
		url:'order_course_api/edit_real_shipping_fee',
...
function load_address () {
	...
	var act = $(':hidden[name=act]').val();	
	if (act=='add') {
		location.href=base_url+"order_course/consignee/"+order_id+'?act=add&address_id='+address_id;
	}else{
		location.href=base_url+"order_course/consignee/"+order_id+'?address_id='+address_id;
	}
