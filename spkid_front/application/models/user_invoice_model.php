...
class User_invoice_model extends CI_Model {
    ...
    public function insert($data) {
        $this->db->insert('ty_user_invoice', $data);
        return $this->db->insert_id();
    }

    public function filter($filter) {
        $query = $this->db_r->get_where('ty_user_invoice', $filter, 1);
        return $query->row();
    }

    public function update($data, $model_id) {
        $this->db->update('ty_user_invoice', $data, array('id' => $model_id));
    }

    public function del($data) {
        $this->db->delete('ty_user_invoice', $data);
    }
