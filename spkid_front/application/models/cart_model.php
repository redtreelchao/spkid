...
class Cart_model extends CI_Model
{
	...
	public function all_cart($filter)
	{
         ...
         * @return type
         */
	public function cart_info ($cart_sn,$gallery=FALSE,$package_product=FALSE)
	{
		$CI = &get_instance();
		$select = "select ct.*,
			p.brand_id,b.brand_name,p.category_id,p.product_sn,p.product_name,p.shop_price,p.shop_price as product_price, p.is_promote,p.promote_start_date,p.promote_end_date,p.promote_price,
			p.market_price, p.limit_num, p.limit_day, c.color_name,s.size_name,
            p.provider_id, pp.display_name as provider_name, pp.shipping_fee_config";
		...
		$sql = $select.$from."where  ct.session_id = ? ". " ORDER BY ct.create_date DESC ";
	...
	public function available_pay_list()
	{
		$sql = "SELECT DISTINCT p.*
			...
			WHERE r.source_id = ? AND r.show_type!=4 AND p.is_discount=0 ORDER BY p.sort_order DESC";
	...
	public function assign_trans($order,$op,$shop_price=NULL)
    {
        ...
        $sql = "SELECT t.depot_id,t.location_id,SUM(t.product_number) AS product_number, t.batch_id, t.consign_price, t.cost_price, t.consign_rate, t.product_cess, t.shop_price
                ...
                GROUP BY l.location_id, t.batch_id HAVING product_number>0 ORDER BY min(t.batch_id) ASC, d.depot_priority;";
