...
class Cart extends CI_Controller {
    ...
    public function index() {
        ...
        if (!$cart_list) {
            $this->load->view('cart/blank_cart', array('title' => '购物车'));
            return;
        }
    ...
    public function update_cart() {
        ...
        if (!$cart_list || !isset($cart_list[$rec_id]))
            sys_msg('由于操作时间过长，您的购物车数据已丢失。', 2);
        $rec = $cart_list[$rec_id];
        if ($rec->package_id && $num != 1)
            sys_msg('礼包商品不允许更改数量', 1);
        ...
        if($cart_list[$rec_id]->limit_num && $cart_list[$rec_id]->limit_day){
            ...
            if($bought_num + $current_num > $cart_list[$rec_id]->limit_num){
                sys_msg("对不起，此商品是限购商品{$cart_list[$rec_id]->limit_day}天内限购{$cart_list[$rec_id]->limit_num}件。", 1);
            }
        }
        ...
        if ($sub->sale_num != -2 && $sub->sale_num < $num)
            sys_msg('对不起，该商品库存不足,请调整购买的数量。', 1);
        ...
        print json_encode(array('err' => 0, 'msg' => '', 'cart' => $cart_list[$rec_id], 'cart_summary' => $cart_summary));
    }
    ...
    public function checkout() {
        ...
        $this->load->vars(array(
            ...
            'pay_list' => $this->cart_model->available_pay_list(),
    ...
    public function proc_checkout() {
        ...
        foreach($cart_summary['product_list'] as $provider_id=>$provider){
            ...
            $order['pay_id'] = $order['order_price'] + $order['shipping_fee'] > $order['paid_price'] ? PAY_ID_ALIPAY : PAY_ID_BALANCE;
        ...
        // 清除购物车
        $this->cart_model->delete_where(array('session_id' => $cart_sn));
        // 清除session
    ...
    public function success($order_ids) {
        global $alipay_bank_list;
        $this->load->model('order_model');
    ...
    public function pay_voucher() {
        ...
        $checkout['payment']['voucher'][$provider_id] = $voucher;
        $this->session->set_userdata('checkout', $checkout);
    ...
    public function unpay_voucher() {
        ...
        $this->session->set_userdata('checkout', $checkout);
        print json_encode(array(array('err'=>0)));
    }
