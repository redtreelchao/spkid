...
class User extends CI_Controller
{
	...
	public function index()
	{
        ...
        if ($user_info->rank_id>0 && $user_info->rank_id<4)
        {
            ...
            $user_info->next_min_points = number_format($arr_user_rank[$user_info->rank_id+1]->min_points-$user_info->paid_money, 2, '.', '');
        ...
        /** 是否完善个人信息*/
        if (empty($user_info->user_name) || empty($user_info->real_name) || empty($user_info->baby_name) || empty($user_info->baby_sex) || empty($user_info->baby_birthday))
        {
...
/*		$baby_birthdayYear = intval(trim($this->input->post('baby_birthdayYear')));
	...
	public function login($register=FALSE)
	{
        ...
        $this->load->view('user/login_win',array(
			'title' => '妈咪树_登录注册',
			'is_register' =>!empty($register)
		));
	...
	public function proc_login() {
	    ...
	    if ($err_msg == '' && $this->user_obj->login($user_name, $password, $remember) == FALSE) {
...
//		    $back_url = 'index';
    ...
    public function proc_register()
    {
        ...
        if (empty($err_msg))
        {
            ...
            $this->session->unset_userdata('back_url');
            echo json_encode(array('error' => 0, 'back_url' => $back_url));
        } else
        {
