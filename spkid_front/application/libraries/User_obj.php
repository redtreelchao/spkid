...
class User_obj {
     ...
     * @return boolean
     */
    public function login($user_name = '', $password = '', $remember = 0)
    {
	    ...
	    $this->CI->session->set_userdata('union_fclub', $user->union_fclub);
            ...
            $this->CI->session->set_userdata('email_validated', $user->email_validated);
...
//	    }
     ...
     * @return boolean
     */
	public function is_email_register($user_email = '')
    {
    	$this->CI->load->model('user_model');
    ...
    public function register($param = array())
    {
		...
		$user_input['password'] = m_encode($param['password']);
		$user_input[$field] = $param['email'];
		//$user_input['last_ip'] = get_real_ip();
	    ...
	    $this->CI->session->set_userdata('union_fclub', '');
            ...
            $this->CI->session->set_userdata('mobile_checked', 0);
		    ...
		    {
		    	@$this->send_sync_sms(array('user_id'=>$this->_user_id,'user_name'=>$param['user_name'],'mobile'=>$param['email'],'password'=>$param['password']),'register_user');
		    }
...
*/
	...
	function send_sync_sms($arg = array(),$send_type = NULL)
	{
		...
		$rs = $this->CI->mobile->send($content,$mobile);
		if ($rs==true)
		{
			$insert_arr['status'] = 1;
		} else
	    ...
	    return $insert_arr['status']==1?true:false;
	}
